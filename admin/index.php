...
<body class="bg-bg text-textMain h-screen flex overflow-hidden font-sans">
        ...
        <div class="p-8 pb-20">
            <?php if(isset($_GET['msg'])): ?>
                <div
                    ...
                    <i data-lucide="check-circle"></i> Action Successful
                </div>
            <?php endif; ?>
            ...
            <?php if ($page == 'Product'): 
                    ...
                    if (isset($_POST['save_fish'])) {
                        ...
                        // NEW: Priority Handling
                        $priority = isset($_POST['priority']) && $_POST['priority'] !== '' ? intval($_POST['priority']) : 999;
                        ...
                        if ($action == 'upload_fish') {
                            ...
                            // NEW: Added priority to INSERT
                            $stmt = $pdo->prepare("INSERT INTO gallery (title, scientific_name, origin, grade, category, is_featured, image_data, priority) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                            $stmt->execute([$title, $scientific, $origin, $grade, $category, $is_featured_requested, $imgData, $priority]);
                        } elseif ($action == 'update_fish') {
                            if (isset($_FILES['image']) && $_FILES['image']['tmp_name']) {
                                $imgData = file_get_contents($_FILES['image']['tmp_name']);
                                // NEW: Added priority to UPDATE with image
                                $stmt = $pdo->prepare("UPDATE gallery SET title=?, scientific_name=?, origin=?, grade=?, category=?, is_featured=?, image_data=?, priority=? WHERE id=?");
                                $stmt->execute([$title, $scientific, $origin, $grade, $category, $is_featured_requested, $imgData, $priority, $id]);
                            } else {
                                // NEW: Added priority to UPDATE without image
                                $stmt = $pdo->prepare("UPDATE gallery SET title=?, scientific_name=?, origin=?, grade=?, category=?, is_featured=?, priority=? WHERE id=?");
                                $stmt->execute([$title, $scientific, $origin, $grade, $category, $is_featured_requested, $priority, $id]);
                            }
                        }
                        $safeRedirect("index.php?page=Product");
                    }
                ...
                ?>
                            ...
                            <div class="lg:col-span-1">
                                <div class="bg-card p-6 rounded-[20px] shadow-sm sticky top-28 border border-gray-100">
                                    ...
                                    <form method="post" enctype="multipart/form-data" class="space-y-4">
                                        <!-- Action Hidden Field -->
                                        <input type="hidden" name="action" value="upload_fish">

                                        <div class="space-y-1">
                                            <label class="text-[10px] font-bold text-secondary uppercase ml-1">Fish Title</label>
                                            <input type="text" name="title" placeholder="e.g. Blue Diamond Discus"
                                        ...
                                        <div class="space-y-1">
                                            ...
                                            <input type="text" name="scientific" placeholder="Symphysodon aequifasciatus"
                                        ...
                                        <div class="grid grid-cols-2 gap-4">
                                            <div class="space-y-1">
                                                ...
                                                <input type="text" name="origin" placeholder="Amazon Basin"
                                                    class="w-full bg-bg p-3 rounded-xl outline-none border border-transparent focus:border-primary transition-all">
                                            </div>
                                            <div class="space-y-1">
                                                <label class="text-[10px] font-bold text-secondary uppercase ml-1">Grade</label>
                                                <input type="text" name="grade" placeholder="Premium / A+"
                                                    class="w-full bg-bg p-3 rounded-xl outline-none border border-transparent focus:border-primary transition-all">
                                            </div>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-2 gap-4">
                                            <div class="space-y-1">
                                                ...
                                                <select name="category" class="w-full bg-bg p-3 rounded-xl outline-none border border-transparent focus:border-primary transition-all">
                                                    <option value="freshwater">Freshwater</option>
                                                    <option value="marine">Marine</option>
                                                </select>
                                            </div>
                                            <div class="space-y-1">
                                                <label class="text-[10px] font-bold text-secondary uppercase ml-1">Priority</label>
                                                <input type="number" name="priority" value="999" placeholder="Order (1=First)"
                                                    class="w-full bg-bg p-3 rounded-xl outline-none border border-transparent focus:border-primary transition-all">
                                            </div>
                                        </div>
                                        ...
                                        <div class="space-y-1">
                                            ...
                                            <div class="bg-bg rounded-xl p-4 border-2 border-dashed border-gray-200 text-center cursor-pointer group hover:border-primary transition-all">
                                                <input type="file" name="image"
                                                    class="w-full text-sm text-secondary file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20" required>
                                            </div>
                                        </div>

                                        <div class="pt-2">
                                            <button type="submit" name="save_fish"
                                                class="w-full bg-primary text-white font-bold py-3 rounded-xl hover:bg-blue-700 transition-all shadow-lg shadow-primary/20">
                                                Save Product
                                            </button>
                                        </div>
                                    </form>
                                ...
                                <div class="bg-card rounded-[20px] shadow-sm overflow-hidden min-h-[600px] border border-gray-100">
                                    <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-white">
                                        <div>
                                            ...
                                            <p class="text-[10px]  italic text-red-600">Priority only work in Index/home page</p>
                                        </div>
                                        <div class="flex gap-2">
                                    ...
                                    <div class="overflow-x-auto">
                                        <table class="w-full text-left border-collapse">
                                            ...
                                            <tbody class="divide-y divide-gray-50">
                                                <?php 
                                                // NEW: ORDER BY Priority ASC first, then ID DESC
                                                $stmt = $pdo->query("SELECT * FROM gallery ORDER BY priority ASC, id DESC");
                    ...
                    <!-- Edit Fish Modal -->
                    <div id="editFishModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50 backdrop-blur-sm" style="display: none;">
                        <div class="bg-white p-8 rounded-3xl w-full max-w-lg shadow-2xl transform scale-100 transition-all">
                            <div class="flex justify-between items-center mb-6">
                                <h3 class="text-xl font-bold text-textMain">Edit Fish Details</h3>
                                <button onclick="closeModal('editFishModal')" class="text-secondary hover:text-primary"><i data-lucide="x"></i></button>
                            </div>
                            <form method="post" enctype="multipart/form-data" class="space-y-4">
                                <input type="hidden" name="action" value="update_fish">
                                <input type="hidden" name="fish_id" id="edit_fish_id">

                                <div class="space-y-1">
                                    <label class="text-[10px] font-bold text-secondary uppercase ml-1">Fish Title</label>
                                    <input type="text" name="title" id="edit_fish_title" class="w-full bg-bg p-3 rounded-xl outline-none border border-transparent focus:border-primary transition-all" required>
                                </div>

                                <div class="space-y-1">
                                    <label class="text-[10px] font-bold text-secondary uppercase ml-1">Scientific Name</label>
                                    <input type="text" name="scientific" id="edit_fish_scientific" class="w-full bg-bg p-3 rounded-xl outline-none border border-transparent focus:border-primary transition-all">
                                </div>

                                <div class="grid grid-cols-2 gap-4">
                                    <div class="space-y-1">
                                        <label class="text-[10px] font-bold text-secondary uppercase ml-1">Origin</label>
                                        <input type="text" name="origin" id="edit_fish_origin" class="w-full bg-bg p-3 rounded-xl outline-none border border-transparent focus:border-primary transition-all">
                                    </div>
                                    <div class="space-y-1">
                                        <label class="text-[10px] font-bold text-secondary uppercase ml-1">Grade</label>
                                        <input type="text" name="grade" id="edit_fish_grade" class="w-full bg-bg p-3 rounded-xl outline-none border border-transparent focus:border-primary transition-all">
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="space-y-1">
                                        <label class="text-[10px] font-bold text-secondary uppercase ml-1">Category</label>
                                        <select name="category" id="edit_fish_category" class="w-full bg-bg p-3 rounded-xl outline-none border border-transparent focus:border-primary transition-all">
                                            <option value="freshwater">Freshwater</option>
                                            <option value="marine">Marine</option>
                                        </select>
                                    </div>
                                    <div class="space-y-1">
                                        <label class="text-[10px] font-bold text-secondary uppercase ml-1">Priority</label>
                                        <input type="number" name="priority" id="edit_fish_priority" class="w-full bg-bg p-3 rounded-xl outline-none border border-transparent focus:border-primary transition-all">
                                    </div>
                                </div>

                                <div class="p-3 rounded-xl bg-amber-50 border border-amber-100">
                                    <label class="flex items-center justify-between cursor-pointer">
                                        <span class="text-sm font-bold text-amber-800">Top 8 (Featured)</span>
                                        <div class="relative inline-block w-10 align-middle select-none">
                                ...
                                <div class="space-y-1">
                                    <label class="text-[10px] font-bold text-secondary uppercase ml-1">Update Image (Optional)</label>
                                    <div class="bg-bg rounded-xl p-4 border-2 border-dashed border-gray-200 text-center cursor-pointer group hover:border-primary transition-all">
                                        <input type="file" name="image" class="w-full text-sm text-secondary file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20">
                                    </div>
                                </div>

                                <div class="pt-4">
                                    <button type="submit" name="save_fish" class="w-full bg-primary text-white py-3 rounded-xl font-bold shadow-lg hover:bg-blue-700 transition-all">Save Changes</button>
                                </div>
                            </form>
                        </div>
                    </div>
                    ...
                    <script>
                        function populateEditForm(fish) {
                            // Fill Modal Inputs
                            document.getElementById('edit_fish_id').value = fish.id;
                            document.getElementById('edit_fish_title').value = fish.title;
                            document.getElementById('edit_fish_scientific').value = fish.scientific_name;
                            document.getElementById('edit_fish_origin').value = fish.origin;
                            document.getElementById('edit_fish_grade').value = fish.grade;
                            document.getElementById('edit_fish_category').value = fish.category;
                            document.getElementById('edit_fish_priority').value = fish.priority || 999;
                            document.getElementById('edit_is_featured').checked = (parseInt(fish.is_featured) === 1);
            ...
            <?php if ($page == 'gallery'): ?>
                <?php
                // --- PERSONAL JOURNEY LOGIC ---

                // FIX: Define safeRedirect inside this scope because this block is isolated
                $safeRedirect = function($url) {
                    if (!headers_sent()) {
                        header("Location: $url");
                    } else {
                ...
                if (isset($_POST['save_journey_content'])) {
                    ...
                    $safeRedirect("index.php?page=gallery");
                }

                // 2. Add Journey Media (with Priority)
                if (isset($_POST['add_journey_media'])) {
                    if (!isset($_POST['media_type'])) $safeRedirect("index.php?page=gallery&error=no_type");
                    
                    $type = $_POST['media_type'];
                    $priority = isset($_POST['priority']) && $_POST['priority'] !== '' ? intval($_POST['priority']) : 999;

                    if ($type === 'image' && !empty($_FILES['media_file']['tmp_name'])) {
                        $pdo->prepare("INSERT INTO personal_journey (media_type, file_data, priority) VALUES ('image', ?, ?)")
                            ->execute([file_get_contents($_FILES['media_file']['tmp_name']), $priority]);
                    } elseif ($type === 'video' && !empty($_FILES['media_file']['tmp_name'])) {
                        $targetDir = "../assets/videos/";
                        if (!is_dir($targetDir)) mkdir($targetDir, 0777, true); 
                        $fileName = time() . '_' . basename($_FILES['media_file']['name']);
                        if(move_uploaded_file($_FILES['media_file']['tmp_name'], $targetDir . $fileName)) {
                            $pdo->prepare("INSERT INTO personal_journey (media_type, video_path, priority) VALUES ('video', ?, ?)")
                                ->execute([$fileName, $priority]);
                        }
                    } elseif ($type === 'youtube' && !empty($_POST['youtube_url'])) {
                        preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $_POST['youtube_url'], $match);
                        $vidId = $match[1] ?? '';
                        if ($vidId) {
                            $pdo->prepare("INSERT INTO personal_journey (media_type, youtube_id, priority) VALUES ('youtube', ?, ?)")
                                ->execute([$vidId, $priority]);
                        }
                    }
                    $safeRedirect("index.php?page=gallery&msg=added");
                }

                // 3. Update Priorities (Reorder)
                if (isset($_POST['update_journey_priorities'])) {
                    if (isset($_POST['priorities']) && is_array($_POST['priorities'])) {
                        foreach ($_POST['priorities'] as $id => $prio) {
                            $id = intval($id);
                            $prio = intval($prio);
                            $pdo->prepare("UPDATE personal_journey SET priority = ? WHERE id = ?")->execute([$prio, $id]);
                        }
                    }
                    $safeRedirect("index.php?page=gallery&msg=reordered");
                }

                // 4. Delete Journey Media
                if (isset($_GET['delete_journey'])) {
                    $pdo->prepare("DELETE FROM personal_journey WHERE id = ?")->execute([$_GET['delete_journey']]);
                    $safeRedirect("index.php?page=gallery&msg=deleted");
                }
                ...
                // NEW: Order by Priority ASC, then ID DESC
                $journey_media = $pdo->query("SELECT * FROM personal_journey ORDER BY priority ASC, id DESC")->fetchAll();

                // Prepare for Alpine (Preview)
                $js_media = [];
                foreach($journey_media as $m) {
                    if($m['media_type'] == 'image') {
                        ...
                        // FIX: Encode filename to handle spaces safely
                        $encodedFile = rawurlencode($m['video_path']);
                        $src = "../assets/videos/{$encodedFile}";
                        $thumb = $src; 
                    } else {
                    ...
                    $js_media[] = ['id' => $m['id'], 'type' => $m['media_type'], 'src' => $src, 'thumb' => $thumb];
                }
                ?>
                ...
                <div class="space-y-6" x-data="{ 
                    ...
                    init() {
                        // WATCHER: Re-load video whenever activeItem changes
                        this.$watch('activeItem', item => {
                            if(item && item.type === 'video') {
                                this.$nextTick(() => {
                                    ...
                                    if(vid) {
                                        vid.src = item.src;
                                        vid.load(); // Forces reload of new source
                                        vid.play().catch(e => { console.log('Autoplay blocked', e) });
                                    }
                                });
                            }
                        });
                        if(this.mediaItems.length > 0) this.setActive(0);
                    }
                }">
                            ...
                            <form method="POST" enctype="multipart/form-data" class="space-y-5">
                                <div class="flex flex-wrap items-center gap-4">
                                    ...
                                    <div class="flex-1 min-w-[120px]">
                                        <input type="number" name="priority" value="999" placeholder="Priority (1=First)" 
                                            class="w-full bg-bg border-none p-2.5 rounded-xl focus:ring-2 focus:ring-primary outline-none text-sm">
                                    </div>
                                </div>
                    ...
                    <!-- NEW: Manage Order & List -->
                    <div class="bg-card p-6 rounded-[20px] shadow-sm border border-gray-100">
                        <div class="flex justify-between items-center mb-6">
                            <h3 class="font-bold text-textMain flex items-center gap-2">
                                <i data-lucide="list-ordered" class="w-5 h-5 text-primary"></i> Manage Order
                            </h3>
                        </div>
                        
                        <form method="POST">
                            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-6">
                                <?php foreach($journey_media as $m): ?>
                                <div class="bg-bg rounded-xl p-3 border border-gray-100 flex flex-col gap-2 group hover:border-primary transition-colors">
                                    <div class="aspect-square rounded-lg overflow-hidden bg-gray-200 relative">
                                        <?php if($m['media_type'] == 'image'): ?>
                                            <img src="../view_image.php?id=<?= $m['id'] ?>&type=journey" class="w-full h-full object-cover">
                                        <?php elseif($m['media_type'] == 'video'): ?>
                                            <!-- Encode URL here too -->
                                            <video src="../assets/videos/<?= rawurlencode($m['video_path']) ?>" class="w-full h-full object-cover"></video>
                                            <div class="absolute inset-0 flex items-center justify-center bg-black/20"><i data-lucide="play" class="text-white w-6 h-6"></i></div>
                                        <?php else: ?>
                                            <img src="https://img.youtube.com/vi/<?= $m['youtube_id'] ?>/hqdefault.jpg" class="w-full h-full object-cover">
                                            <div class="absolute inset-0 flex items-center justify-center bg-red-600/20"><i data-lucide="youtube" class="text-white w-6 h-6"></i></div>
                                        <?php endif; ?>
                                        
                                        <!-- Delete Btn -->
                                        <a href="?page=gallery&delete_journey=<?= $m['id'] ?>" onclick="return confirm('Remove this item?')" 
                                        class="absolute top-1 right-1 bg-red-500 text-white p-1 rounded shadow-sm opacity-0 group-hover:opacity-100 transition-opacity">
                                            <i data-lucide="x" class="w-3 h-3"></i>
                                        </a>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span class="text-[10px] font-bold text-secondary uppercase">Order</span>
                                        <input type="number" name="priorities[<?= $m['id'] ?>]" value="<?= intval($m['priority']) ?>" 
                                            class="w-full bg-white border border-gray-200 rounded px-2 py-1 text-xs focus:border-primary outline-none font-mono text-center">
                                    </div>
                                </div>
                                <?php endforeach; ?>
                            </div>
                            <?php if(count($journey_media) > 0): ?>
                            <div class="flex justify-end">
                                <button type="submit" name="update_journey_priorities" class="bg-gray-800 text-white px-6 py-2 rounded-xl text-sm font-bold hover:bg-black transition-colors flex items-center gap-2">
                                    <i data-lucide="save" class="w-4 h-4"></i> Save New Order
                                </button>
                            </div>
                            <?php else: ?>
                                <p class="text-center text-gray-400 text-sm italic py-8">No media items found.</p>
                            <?php endif; ?>
                        </form>
                    </div>

                    <!-- Cinematic Preview (Alpine) -->
                    <div class="bg-card p-6 rounded-[32px] shadow-sm border border-gray-100 overflow-hidden">
                        <div class="flex justify-between items-center mb-6">
                            <h3 class="font-bold text-textMain flex items-center gap-2">
                                <i data-lucide="monitor" class="w-5 h-5 text-primary"></i> Live Preview
                            </h3>
                        </div>
                        
                        <div class="bg-[#001f2bff] rounded-[32px] overflow-hidden aspect-video flex items-center justify-center relative shadow-2xl group/viewer">
                            <template x-if="activeItem">
                                <div class="w-full h-full flex items-center justify-center animate-viewer">
                                    <img x-show="activeItem.type === 'image'" :src="activeItem.src" class="max-w-full max-h-full object-contain select-none">
                                    
                                    <!-- UPDATED VIDEO TAG: Added x-ref and removed x-effect (using watcher now) -->
                                    <video x-ref="player" x-show="activeItem.type === 'video'" :src="activeItem.src" class="max-w-full max-h-full" controls autoplay muted loop playsinline></video>
                                    
                                    <iframe x-show="activeItem.type === 'youtube'" :src="activeItem.src" class="w-full h-full" frameborder="0" allowfullscreen></iframe>
                                    
                                    <button @click="prev()" class="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-md text-white flex items-center justify-center transition-all opacity-0 group-hover/viewer:opacity-100 z-20"><i data-lucide="chevron-left"></i></button>
                                    <button @click="next()" class="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-md text-white flex items-center justify-center transition-all opacity-0 group-hover/viewer:opacity-100 z-20"><i data-lucide="chevron-right"></i></button>
                                </div>
                            </template>
                            <template x-if="!activeItem">
                                <div class="text-white/30 flex flex-col items-center">
                                    <i data-lucide="clapperboard" class="w-16 h-16 mb-4 opacity-20"></i>
                                    <p class="text-sm font-bold tracking-widest uppercase">Admin Preview</p>
                                </div>
                            </template>
                        </div>
                        
                        <!-- Thumbs (Optional, kept minimal) -->
                        <div class="flex gap-2 mt-4 overflow-x-auto pb-2 journey-scrollbar">
                            <template x-for="(item, index) in mediaItems" :key="item.id">
                                <div @click="setActive(index)" class="w-20 h-14 rounded-lg overflow-hidden shrink-0 cursor-pointer border-2 transition-all" :class="currentIndex === index ? 'border-primary' : 'border-transparent opacity-50 hover:opacity-100'">
                                    <!-- UPDATED: Thumbnail logic to handle video icons instead of broken images -->
    ...
    <script>
        ...
        const tinymceConfig = {
            selector: '.tinymce',
            license_key: 'gpl',
            // Only allow basic text plugins (removed image, media, table, etc.)
            plugins: 'autolink lists wordcount',
            // Disable the menu bar (File, Edit, View, etc.)
            menubar: false,
            // Disable the toolbar completely to allow only text entry
            toolbar: false,
